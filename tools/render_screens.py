...
def phone_doc(s):
    return f'''<!doctype html><html><head><meta charset="utf-8"><link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Poppins:wght@500;600;700&family=IBM+Plex+Mono:wght@400;500&display=swap"><link rel="stylesheet" href="app.css"><style>body{{width:400px;height:820px;background:#fff;font-size:14px}} .pa{{display:flex;flex-direction:column;height:100%}} .pa .st{{display:flex;justify-content:space-between;padding:12px 18px 0;font-family:var(--mono);font-size:11px;color:var(--muted)}} .pa .hd{{padding:12px 18px 10px;display:flex;align-items:center;gap:10px;border-bottom:1px solid var(--line)}} .pa .hd b{{font-size:17px}} .pa .bd{{flex:1;overflow:hidden;padding:14px 18px;display:flex;flex-direction:column;gap:12px}} .pa .tb{{display:grid;grid-template-columns:repeat(4,1fr);border-top:1px solid var(--line);padding:10px 0 18px;text-align:center;font-family:var(--mono);font-size:9.5px;letter-spacing:.06em;text-transform:uppercase;color:var(--muted)}} .pa .tb b{{display:block;font-size:18px;margin-bottom:2px;font-family:var(--display)}} .pa .tb .on{{color:var(--red)}} {s.get('css','')}</style></head><body><div class="pa"><div class="st"><span>9:41</span><span>●●● 5G ▮</span></div>{s['body']}</div></body></html>'''
def main(slug):
    ...
    for i, s in enumerate(m.SCREENS, 1):
        ...
        open(html, 'w').write(phone_doc(s) if getattr(m, 'MODE', '') == 'phone' else shell(m.APP, s))
        try: subprocess.run([CHROME, '--headless=new', '--disable-gpu', '--hide-scrollbars', '--force-device-scale-factor=1', '--virtual-time-budget=6000', '--timeout=25000', ('--window-size=400,820' if getattr(m, 'MODE', '') == 'phone' else '--window-size=1440,900'), f'--screenshot={png}', 'file://' + html], capture_output=True, timeout=60)
        ...
        im = Image.open(png).convert('RGB'); im.save(os.path.join(out, name + '.webp'), quality=84, method=6); (im.resize((240, 492), Image.LANCZOS) if getattr(m, 'MODE', '') == 'phone' else im.resize((480, 300), Image.LANCZOS)).save(os.path.join(out, name + '-thumb.webp'), quality=78)
        meta.append({'file': name, 'title': s['title'], 'caption': s['caption'], 'hl': s.get('hl')})
        print('rendered', name)
    if getattr(m, 'MODE', '') == 'phone':
        json.dump({'app': m.APP['brand'], 'client': m.APP['client'], 'screens': meta, 'mode': 'phone'}, open(os.path.join(ROOT, 'src', 'data', 'screens', slug + '.json'), 'w'), indent=1); print('done', slug); return
